...
sequenceDiagram
    ...
    DET->>DET: ≥3 → medium(3~4) / high(≥5)（detection.py:124-129）
...
| 触发阈值 | `device_count >= 3` | `:120-121` |
| 级别 | `3 <= device_count < 5` → `medium`；`device_count >= 5` → `high` | `:124-129` |
...
同上结构；窗口 1 小时；以 `(user_agent, ip_address)` 去重计数；`≥3` 触发（`:120-121`），`3~4` medium、`≥5` high。
